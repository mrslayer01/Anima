...
//#region Advanatage/Disadvantage/Elan
...
// -----------------------------
  ...
  html.find(".delete-elan").on("click", async (event) => {
    ...
    const deletedName = elans[index]?.name;
...
//#region ELAN fix to stop breaking nested arrays.
//With nested arrays, if you don't capture the data, foundry overwrites the entire object.
...
html.find(".languages-level-input").on("change", async (event) => {
  ...
  html.find(".delete-languages").on("click", async (event) => {
    ...
    const deletedName = lang[index]?.name;
